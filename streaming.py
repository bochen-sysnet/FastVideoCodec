        ...
        for begin in range(0,L,GoP):
            with torch.no_grad():
                ...
                if x_GoP.size(0)>fP+1:
                    ...
                    bpp_act_list1 = []
                    for i in range(1,B):
                        mv_string,res_string,bpp_act,_,mv_size,res_size = model.compress(x_ref, x_b[i:i+1], hidden, False)
                        x_ref,hidden = model.decompress(x_ref, mv_string, res_string, hidden, False, mv_size, res_size)
