...
class LSVC(nn.Module):
    def __init__(self, name, loss_type='P', compression_level=3):
        ...
        self.opticFlow = ME_Spynet()
        self.Q = None
        mv_attn1 = ('-MA' in name) or ('-A' in name) 
        mv_attn2 = ('-A2' in name) 
        self.mvEncoder = Analysis_mv_net(useAttn=mv_attn1,out_channels=out_channel_M)
        self.mvDecoder = Synthesis_mv_net(useAttn=mv_attn2,in_channels=out_channel_M)
        res_attn1 = ('-RA' in name) or ('-A' in name)
        res_attn2 = ('-A2' in name)
        self.resEncoder = Analysis_net(useAttn=res_attn1)
        self.resDecoder = Synthesis_net(useAttn=res_attn2)
        self.respriorEncoder = Analysis_prior_net(useAttn=res_attn1)
        self.respriorDecoder = Synthesis_prior_net(useAttn=res_attn2)
