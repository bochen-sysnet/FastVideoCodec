...
class SPVC(nn.Module):
    def __init__(self, name, channels=128, noMeasure=True, loss_type='P', 
        ...
        self.warpnet = Warp_net()
        if '-G' in self.name:
            self.globalnet = Warp_net(in_channels=3*6,out_channels=3*6)
        if '96' in self.name:
            channels = 96
    ...
    def forward(self, x):
        ...
        if '-G' in self.name:
            if bs<6:
                pad = 6-bs
                inputfeature = torch.cat((MC_frames, MC_frames[-1].repeat(pad,1,1,1)), 0)
            else:
                inputfeature = MC_frames
            inputfeature = inputfeature.view(1,6*c,h,w)
            prediction = self.globalnet(inputfeature) + inputfeature
            prediction = prediction.view(6,c,h,w)
