...
def get_codec_model(name, loss_type='P', compression_level=2, noMeasure=True, use_split=True, num_views=0, resilience=0):
    if name in ['RLVC','DVC','RLVC2']:
        ...
        model_codec = MCVC(name, loss_type='P', compression_level=compression_level, num_views=num_views, resilience=resilience)
...
def compress_whole_video(name, raw_clip, Q, width=256,height=256, GOP=16, frame_comb=0):
    ...
    if isinstance(raw_clip,list):
        ...
        if frame_comb == 0:
            ...
            for g in range(raw_clip.size(0)):
                Y1_raw = torch.cat([raw_clip[g][v].unsqueeze(0) for v in range(raw_clip.size(0))])
                Y1_com = torch.cat([clip[v + g * raw_clip.size(1)].unsqueeze(0) for v in range(raw_clip.size(0))])
                psnr_list += [PSNR(Y1_raw, Y1_com)]
                msssim_list += [MSSSIM(Y1_raw, Y1_com)]
                bpp_act_list += torch.FloatTensor([bpp])
        elif frame_comb == 3:
...
class ELFVC(ScaleSpaceFlow):
    def __init__(
        ...
        self.super_prec = True if '-SP' in name else False
        self.compression_level = compression_level
        self.loss_type = loss_type
        init_training_params(self)
        self.spstage = 1 # amybe fix this
        self.alpha = 0
        ...
        self.res_hyperprior = Hyperprior(super_prec=self.super_prec, sp=res_sp)
        self.motion_hyperprior = Hyperprior(super_prec=self.super_prec, sp=motion_sp)
        self.name = name
    ...
    def optim_parameters(self, epoch, learning_rate, weight_decay):
        ...
        print("Epoch:",epoch,'stage:',self.spstage, 'learning rate:', lr, self.alpha)
