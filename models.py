    ...
    def forward(self, input_image, referframe, priors):
        ...
        if not self.useSSF:
            ...
            if self.training:
                if self.useER: 
                    ...
                    quant_noise_mv = (torch.randn_like(quant_noise_mv)*.1).clamp(-.5, .5)
                elif self.useE2R:
        ...
        if not self.useSTE:
            if self.training:
                if self.useER: 
                    ...
                    quant_noise_feature = (torch.randn_like(quant_noise_feature)*.1).clamp(-.5, .5)
                elif self.useE2R:
                    quant_noise_feature = self.resErrNet((input_residual))
        ...
        if self.training:
            if self.useER: 
                ...
                quant_noise_z = (torch.randn_like(quant_noise_z)*.1).clamp(-.5, .5)
            elif self.useE2R:
                quant_noise_z = self.respriorErrNet((feature))
