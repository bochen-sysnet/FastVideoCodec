    ...
    def forward(self, input_image, referframe, priors):
        ...
        if not self.useSSF:
            ...
            if self.training:
                if self.useER:
                    ...
                    max_noise = torch.abs(quant_noise_mv).max()
                    quant_noise_mv = quant_noise_mv * 0.5/max_noise
                elif self.useE2R:
        ...
        if not self.useSTE:
            if self.training:
                if self.useER: 
                    ...
                    max_noise = torch.abs(quant_noise_feature).max()
                    quant_noise_feature = quant_noise_feature * 0.5/max_noise
                elif self.useE2R:
                    quant_noise_feature = self.resErrNet((input_residual))
        ...
        if self.training:
            if self.useER: 
                ...
                max_noise = torch.abs(quant_noise_z).max()
                quant_noise_z = quant_noise_z * 0.5/max_noise
            elif self.useE2R:
                quant_noise_z = self.respriorErrNet((feature))
